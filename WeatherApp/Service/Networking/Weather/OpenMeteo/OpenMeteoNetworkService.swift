...
final class OpenMeteoNetworkService: WeatherNetworkServiceProtocol {
    ...
    func getWeatherIn(
        ...
        APINetworkManager.request(
            to: OpenMeteoAPIEndpoint.getWeatherIn(
                longitude: longitude,
                latitude: latitude
            )
        ) { (result: Result<OpenMeteoWeatherResponse, APIErrors>) in
            switch result {
            case .success(let openWeatherResponse):
                if let temperature = openWeatherResponse.current?.temperature2M,
                    let weatherCondition = openWeatherResponse.current?.weatherCode?.description {
                    let weatherModel = WeatherModelAfterDTO(
    ...
    func getWeatherForecast(
        ...
        APINetworkManager.request(to: OpenMeteoAPIEndpoint.getWeatherWithForecast(
            longitude: longitude,
            latitude: latitude)
        ) { (result: Result<OpenMeteoWeatherResponse, APIErrors>) in
            ...
            case .success(let weatherResponse):
                guard let hourlyTemperature = weatherResponse.hourly?.temperature2M,
                      ...
                      let hourlyTimes = weatherResponse.hourly?.time
                else { completion(.failure(.noDataInResponse)); return }
                ...
                guard let dailyMinTemperatures = weatherResponse.daily?.temperature2MMin,
                      ...
                      let dailyTimes = weatherResponse.daily?.time
                else { completion(.failure(.noDataInResponse)); return }
                ...
                guard let currentTemperature = weatherResponse.current?.temperature2M,
                      let currentWeatherCode = weatherResponse.current?.weatherCode
                else { completion(.failure(.noDataInResponse)); return }

                let model = WeatherWithForecastAfterDTO(
                    temperature: currentTemperature,
                    weatherCondition: currentWeatherCode.description,
