...
final class CitySearchPresenter {
    ...
    init(router: CitySearchRouter,
         weatherService: WeatherNetworkServiceProtocol,
         locationService: LocationServiceProtocol,
         geocoderNetworkService: GeocoderNetworkServiceProtocol,
         geoObjectPersistenceService: GeoObjectServiceProtocol) {
        self.router = router
...
extension CitySearchPresenter: CitySearchPresenterProtocol {
    func viewDidLoad(with view: CitySearchViewProtocol) {
        ...
        view.configureCurrentWeatherSection(
            with: CurrentWeatherCellModel(
                city: staticStrings.undefinedLocation,
                temperatureText: staticStrings.undefinedTemeperature,
                condition: staticStrings.undefinedTemeperature
            )
        )
    ...
    func onCityDidTapped(model: PlaceCellModel) {
        ...
        router.showWeatherFor(
            city: model.cityName,
            latitude: model.latitude,
            longitude: model.longitude,
            weatherNetworkService: self.weatherNetworkService)
    ...
    func onCurrentCityDidTapped() {
        ...
        if let location = locationService.getLocation(),
            ...
            city != staticStrings.undefinedLocation {
            router.showWeatherFor(city: city,
...
private extension CitySearchPresenter {
    ...
    private func saveToPersistentGeoObject(title: String,
                                           subtitle: String,
                                           longitude: Decimal,
                                           latitude: Decimal,
                                           uri: String) {
        geoObjectPersistenceService.createGeoObject(title: title,
                                                    subtitle: subtitle,
                                                    longitude: longitude,
                                                    latitude: latitude,
                                                    uri: uri)
    ...
    private func onFavoriteButtonDidTapped(model: PlaceCellModel) {
        if model.isFavorite {
    ...
    private func createNewFavorite(model: PlaceCellModel) {
        saveToPersistentGeoObject(title: model.cityName,
