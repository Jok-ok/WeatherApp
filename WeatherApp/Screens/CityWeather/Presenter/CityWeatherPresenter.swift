...
final class CityWeatherPresenter: CityWeatherPresenterProtocol {
    ...
    init(
        router: CityWeatherRouterProtocol,
...
private extension CityWeatherPresenter {
    ...
    func requestWeather() {
        weatherNetworkService?.getWeatherForecast(
    ...
    func showDailyWeather() {
        ...
        let dailyForecastMaxTemepraturePoints = getChartPoints(
            from: weatherForecastModel.dailyForecast.times,
            values: weatherForecastModel.dailyForecast.maxTemperatures,
            formatter: dateFormetter
        )

        datasets.append(
            ChartCellModel.DataSet(
                points: dailyForecastMaxTemepraturePoints,
                colorSceme: .orange
            )
        )

        let dailyForecastMinTemepraturePoints = getChartPoints(
            from: weatherForecastModel.dailyForecast.times,
            values: weatherForecastModel.dailyForecast.minTemperatures,
            formatter: dateFormetter
        )

        datasets.append(
            ChartCellModel.DataSet(
                points: dailyForecastMinTemepraturePoints,
                colorSceme: .standart
            )
        )

        let chartCellModel = ChartCellModel(linesDatasets: datasets,
                                            title: staticStrings.minMaxTemeperature)
    ...
    func showHourlyyWeather() {
        ...
        let hourlyForecasTemepraturePoints = getChartPoints(
            from: weatherForecastModel.hourlyForecast.times,
            values: weatherForecastModel.hourlyForecast.temperature,
            formatter: timeFormatter)

        let hourlyForecasPreciptationProbPoints = getChartPoints(
            from: weatherForecastModel.hourlyForecast.times,
            values: weatherForecastModel.hourlyForecast.precipitationProbability.map(
                { Double($0) }
            ),
            formatter: timeFormatter
        )
        ...
        datasets.append(
            ChartCellModel.DataSet(
                points: hourlyForecasTemepraturePoints,
                colorSceme: .standart
            )
        )

        let temperaTureChartCellModel = ChartCellModel(
            linesDatasets: datasets,
            title: staticStrings.temperature
        )
        datasets.removeAll()
        datasets.append(
            ChartCellModel.DataSet(
                points: hourlyForecasPreciptationProbPoints,
                colorSceme: .standart
            )
        )
        let preceptationProbability = ChartCellModel(
            linesDatasets: datasets,
            title: staticStrings.preceptationProbability
        )

        view?.configureHourlyWeatherSection(with: [temperaTureChartCellModel, preceptationProbability])
    }

    func getChartPoints(from time: [Date], values: [Double], formatter: DateFormatter) -> [ChartCellModel.Point] {
        ...
        let points = zippedValues.map({ ChartCellModel.Point(xVar: $0, yVar: $1) })
    ...
    func showError(with message: String) {
        router?.showAlert(
            with: staticStrings.errorAlertControllerTitle,
            message: message,
            cancelButtonTitle: staticStrings.errorCancelButtonText
        )
    }
