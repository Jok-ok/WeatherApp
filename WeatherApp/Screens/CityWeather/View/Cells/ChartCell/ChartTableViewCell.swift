    ...
    private func drawChart() {
        ...
        for dataset in model.linesDatasets {
            ...
            for (index, point) in dataset.points.enumerated() {
                let lineEntry = ChartDataEntry(x: Double(index), y: point.yVar)
            ...
            xAxis.valueFormatter = IndexAxisValueFormatter(values: dataset.points.map { $0.xVar })
            xAxis.labelPosition = .bottom
            xAxis.labelTextColor = .getAppColor(.accentOp)!

            if let gradient = getColorGradient(
                firsColor: dataset.colorSceme.accentColor,
                secondColor: dataset.colorSceme.secondaryColor) {
                lineChartDataSet.fill = LinearGradientFill(gradient: gradient,
                                                           angle: 270)
            }
